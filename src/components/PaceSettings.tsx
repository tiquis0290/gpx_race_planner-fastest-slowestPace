...
const PaceSettings: React.FC = () => {
  ...
  return (
    <Card title={t.paceCard} className="mb-3">
      <div className="mb-3">
        <SelectButton value={targetMode} options={modeOptions} onChange={(e) => dispatch(setTargetMode(e.value))} />
      </div>
      <div className="pace-inputs">
        <div className="pace-input">
          <label className="block mb-1 text-sm font-medium">{t.paceLabel}</label>
          <InputText value={paceInput} onChange={(e) => handlePaceChange(e.target.value)} placeholder="5:30" className={paceError ? 'p-invalid' : ''} />
        ...
        <div className="pace-input">
          ...
          <InputText value={timeInput} onChange={(e) => handleTimeChange(e.target.value)} placeholder="1:45:00" className={timeError ? 'p-invalid' : ''} />
