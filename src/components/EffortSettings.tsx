...
const EffortSettings: React.FC = () => {
  ...
  return (
    <Card title={t.effortCard} className="mb-3">
      <div className="effort-section">
        ...
        <Dropdown
          ...
          onChange={(e) => dispatch(setEffortModel(e.value as EffortModel))}
        />
      </div>
      ...
      {effortModel === 'power' && (
        <div className="effort-section">
          ...
          <InputNumber
            value={powerExponent}
            onValueChange={(e) => dispatch(setPowerExponent(e.value ?? 1.5))}
      ...
      {effortModel !== 'minetti' && (
        <div className="effort-cost-row">
          <div className="effort-cost-item">
            ...
            <InputNumber value={uphillCost} onValueChange={(e) => dispatch(setUphillCost(e.value ?? 15))} min={0} max={60} step={1} showButtons suffix=" s" />
          </div>
          <div className="effort-cost-item">
            <label className="block mb-1 text-sm font-medium effort-cost-item__label">
              {t.downhillLabel}
            ...
            <InputNumber value={downhillBenefit} onValueChange={(e) => dispatch(setDownhillBenefit(e.value ?? 8))} min={0} max={40} step={1} showButtons suffix=" s" />
          </div>
        </div>
      )}
      ...
      <div className="effort-section">
        ...
        <SelectButton value={splitStrategy} options={splitOptions} onChange={(e) => dispatch(setSplitStrategy(e.value))} />
      </div>
      ...
      {splitStrategy !== 'even' && (
        <div>
          ...
          <Slider value={Math.round(splitStrength * 100)} onChange={(e) => dispatch(setSplitStrength((e.value as number) / 100))} min={0} max={20} step={1} />
        </div>
      )}
    </Card>
  );
